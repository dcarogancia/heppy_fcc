...
class BackgroundBs2DsDsKWithDs2PiPiPiPiAnalyzer(CommonAnalyzer):
    """
    ...
    def __init__(self, cfg_ana, cfg_comp, looper_name):
        ...
        super(BackgroundBs2DsDsKWithDs2PiPiPiPiAnalyzer, self).__init__(cfg_ana, cfg_comp, looper_name)
        ...
        self.mc_truth_tree.var('pi0_dplus_px')
        self.mc_truth_tree.var('pi0_dplus_py')
        self.mc_truth_tree.var('pi0_dplus_pz')
        ...
        self.mc_truth_tree.var('pi0_dminus_px')
        self.mc_truth_tree.var('pi0_dminus_py')
        self.mc_truth_tree.var('pi0_dminus_pz')
                ...
                if abs(ptc_gen1.p.pz) / ptc_gen1.p.absvalue() <= math.cos(math.pi / 12):
                    ...
                    if pb > 25.: # Select only events with large momentum of the B
                        ...
                        if pvsv_distance > 1.: # Select only events with long flight distance of the B
                                ...
                                if ptc_gen2.start_vertex == b.end_vertex and abs(ptc_gen2.p.pz) / ptc_gen2.p.absvalue() <= math.cos(math.pi / 12):
                                    ...
                                    if ptc_gen2.pdgid == 431:
                                        ...
                                        for ptc_gen3 in ptcs:
                                            if ptc_gen3.start_vertex == dplus.end_vertex  and abs(ptc_gen3.p.pz) / ptc_gen3.p.absvalue() <= math.cos(math.pi / 12):
                                                # looking for pi+/-
                                    ...
                                    if ptc_gen2.pdgid == -431:
                                        ...
                                        pis_dminus = []
                                        for ptc_gen3 in ptcs:
                                            if ptc_gen3.start_vertex == dminus.end_vertex and abs(ptc_gen3.p.pz) / ptc_gen3.p.absvalue() <= math.cos(math.pi / 12):
                                                # looking for pi+/-
                                                if abs(ptc_gen3.pdgid) == 211:
                            ...
                            if b is not None and kstar is not None and k is not None and pi_kstar is not None and dplus is not None and dminus is not None and pi1_dplus is not None and pi2_dplus is not None and pi3_dplus is not None and pi0_dplus is not None and pi1_dminus is not None and pi2_dminus is not None and pi3_dminus is not None and pi0_dminus is not None:
                                ...
                                if max_svtv_distance > 0.5: # select only events with long flight distance of tau
                                    ...
                                    self.mc_truth_tree.fill('pi0_dplus_px', pi0_dplus.p.px)
                                    self.mc_truth_tree.fill('pi0_dplus_py', pi0_dplus.p.py)
                                    self.mc_truth_tree.fill('pi0_dplus_pz', pi0_dplus.p.pz)
                                    ...
                                    self.mc_truth_tree.fill('pi0_dminus_px', pi0_dminus.p.px)
                                    self.mc_truth_tree.fill('pi0_dminus_py', pi0_dminus.p.py)
                                    self.mc_truth_tree.fill('pi0_dminus_pz', pi0_dminus.p.pz)
                                    ...
                                    self.tree.fill('pi3_tauplus_py', self.cfg_ana.momentum_smearer(pi3_tauplus.p.py, pi3_tauplus.p) if self.cfg_ana.smear_momentum else pi3_tauplus.p.py)
                                    self.tree.fill('pi3_tauplus_pz', self.cfg_ana.momentum_smearer(pi3_tauplus.p.pz, pi3_tauplus.p) if self.cfg_ana.smear_momentum else pi3_tauplus.p.pz)
