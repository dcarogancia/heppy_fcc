...
class BdDstMuNuKPiPiAnalyzer(Analyzer):
    def beginLoop(self, setup):
        ...
        super(BdDstMuNuKPiPiAnalyzer, self).beginLoop(setup)
        #self.rootfile = TFile('/'.join([self.dirName, 'Bd2DmunuKpipi-100k.root']), 'recreate')
        
        self.rootfile = TFile('/'.join([self.dirName, 'Bd_Dstmunu_Kpipi_signal_evtpdl2019-1M.root']), 'recreate')
        ...
        for ptc_gen in ptcs:
            ...
            if (abs(ptc_gen.pdgid) == 511 and ptc_gen.start_vertex != ptc_gen.end_vertex):
                ...
                if len(plist_Bd_Dst) == 1 and len(plist_Bd_mu) == 1 and self.Bd_Dstmu_counter < 1000000:
                    ...
                    #print 'D daughters ' , plist_Dst_D0[0].pdgid, plist_Dst_D0[1].pdgid
                    ...
                    D_pi2_px, D_pi2_py, D_pi2_pz = plist_Bd_pi[0].p.px, plist_Bd_pi[0].p.py, plist_Bd_pi[0].p.pz
                    D_mu_px, D_mu_py, D_mu_pz = plist_Bd_mu[0].p.px, plist_Bd_mu[0].p.py, plist_Bd_mu[0].p.pz
                    
                    D_K_p, D_pi1_p, D_pi2_p, D_mu_p = plist_Dst_D0[0].p.absvalue(), plist_Dst_D0[1].p.absvalue(), plist_Bd_pi[0].p.absvalue(), plist_Bd_mu[0].p.absvalue()
                    
                    D_K_En, D_pi1_En, D_pi2_En, D_mu_En = plist_Dst_D0[0].energy, plist_Dst_D0[1].energy, plist_Bd_pi[0].energy, plist_Bd_mu[0].energy
                    ...
                    D_pi2 = copy.deepcopy(plist_Bd_pi[0])
                    ...
                    D_pi2_pT = np.sqrt(plist_Bd_pi[0].p.px**2 + plist_Bd_pi[0].p.py**2)
                    ...
                    #Bd_Kpipimu_smass = np.sqrt((Dst_Kpipimu_sEn + Bd_X_sEt)**2 - Bd_Kpipimu_spt)
