...
class LbDsstLbcXKKPiAnalyzer(Analyzer):
    def beginLoop(self, setup):
        ...
        #self.rootfile = TFile('/'.join([self.dirName, 'Lb_LcDsX_KKpi_bkg_evtpdl2019_0.5mmBfdcut-137k.root']), 'recreate')
        self.rootfile = TFile('/'.join([self.dirName, 'Lb_LcDsX_KKpi_bkg_evtpdl2019-137k.root']), 'recreate')
            ...
            if (abs(ptc_gen.pdgid) == 5122 and ptc_gen.start_vertex != ptc_gen.end_vertex):
                ...
                for ptc_gen1 in ptcs:   
                    ...
                    if (ptc_gen1.start_vertex == sv_mctruth and ptc_gen1.start_vertex != ptc_gen1.end_vertex):
                        ...
                        if abs(ptc_gen1.pdgid) not in [4122,431,5122]:
                ...
                #if len(plist_Bd_Ds) == 1 and len(plist_L0b_Lc) == 1 and Bd_fd > 0.5 and self.DDs_counter < 137000:
                if len(plist_Bd_Ds) == 1 and len(plist_L0b_Lc) == 1 and self.DDs_counter < 137000:
                    ...
                    Bd_pt = np.dot([plist_Bd_Ds[0].p.px + plist_L0b_Lc[0].p.px + Bd_X_px, plist_Bd_Ds[0].p.py + plist_L0b_Lc[0].p.py + Bd_X_py, plist_Bd_Ds[0].p.pz + plist_L0b_Lc[0].p.pz + Bd_X_pz],[plist_Bd_Ds[0].p.px + plist_L0b_Lc[0].p.px + Bd_X_px, plist_Bd_Ds[0].p.py + plist_L0b_Lc[0].p.py + Bd_X_py, plist_Bd_Ds[0].p.pz + plist_L0b_Lc[0].p.pz + Bd_X_pz])
                    
                    
                    
                    Bd_mass = np.sqrt((Bd_Ds_En + plist_L0b_Lc[0].energy + Bd_X_Et)**2 - Bd_pt)                                     
                    
                    Bd_spt = np.dot([Bd_Ds.p.px + L0b_Lc.p.px + Bd_X_spx, Bd_Ds.p.py + L0b_Lc.p.py + Bd_X_spy, Bd_Ds.p.pz + L0b_Lc.p.pz + Bd_X_spz],[Bd_Ds.p.px + L0b_Lc.p.px + Bd_X_spx, Bd_Ds.p.py + L0b_Lc.p.py + Bd_X_spy, Bd_Ds.p.pz + L0b_Lc.p.pz + Bd_X_spz])
                    
                    Bd_Ds_sEn = np.sqrt(Bd_Ds.p.absvalue()**2 + Bd_Ds.mass**2)
                    
                    Bd_D_sEn = np.sqrt(L0b_Lc.p.absvalue()**2 + L0b_Lc.mass**2)
    ...
    def write(self, unusefulVar):
        ...
        print('D(+/-) meson events: {}'.format(self.Lc_counter)) 
