				...
				if pb > 25.: # Select only events with large momentum of the B
					...
					if pvsv_distance > 1.: # Select only events with long flight distance of the B
						...
						for ptc_gen2 in ptcs:
							# looking for tauplus
							if ptc_gen2.pdgid == -15 and ptc_gen2.start_vertex == b.end_vertex:
								tauplus = copy.deepcopy(ptc_gen2) # copy is needed in order to keep initial particle properties after smearing
								...
								for ptc_gen3 in ptcs:
									if abs(ptc_gen3.pdgid) == 211 and ptc_gen3.start_vertex == tauplus.end_vertex:
							...
							if ptc_gen2.pdgid == 15 and ptc_gen2.start_vertex == b.end_vertex:
								tauminus = copy.deepcopy(ptc_gen2) # copy is needed in order to keep initial particle properties after smearing
								...
								pis_tauminus = list([])
								for ptc_gen3 in ptcs:
									if abs(ptc_gen3.pdgid) == 211 and ptc_gen3.start_vertex == tauminus.end_vertex:
						...
						if max(svtv_tauplus_distance, svtv_tauminus_distance) > 0.5 and b.is_valid() and k.is_valid() and pi_k.is_valid() and pi1_tauplus.is_valid() and pi2_tauplus.is_valid() and pi3_tauplus.is_valid() and pi1_tauminus.is_valid() and pi2_tauminus.is_valid() and pi3_tauminus.is_valid(): # Select only events with long flight distance of tau
