...
class BsDsstMuNuKPiPiAnalyzer(Analyzer):
    def beginLoop(self, setup):
        ...
        super(BsDsstMuNuKPiPiAnalyzer, self).beginLoop(setup)
        #self.rootfile = TFile('/'.join([self.dirName, 'Bd2DmunuKpipi-100k.root']), 'recreate')
        
        self.rootfile = TFile('/'.join([self.dirName, 'Bs_Dsstmunu_KKpi_signal_evtpdl2019-1M.root']), 'recreate')
        ...
        for ptc_gen in ptcs:
            ...
            if (abs(ptc_gen.pdgid) == 531 and ptc_gen.start_vertex != ptc_gen.end_vertex):
                ...
                if len(plist_Bs_Ds) == 1 and len(plist_Bs_mu) == 1 and self.Bs_Dsmu_counter < 1000000:
                    ...
                    Bs_KKpimu_smcorr = np.sqrt(Bs_KKpimuX_smass**2 + KKpimu_spTmis_FdB) + np.sqrt(KKpimu_spTmis_FdB)
