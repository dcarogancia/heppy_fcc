                ...
                if abs(ptc_gen1.p.pz) / ptc_gen1.p.absvalue() <= math.cos(math.pi / 12):
                    ...
                    if pb > 25.: # Select only events with large momentum of the B
                        ...
                        if pvsv_distance > 1.: # Select only events with long flight distance of the B
                            ...
                            for ptc_gen2 in ptcs:
                                if ptc_gen2.start_vertex == b.end_vertex and abs(ptc_gen2.p.pz) / ptc_gen2.p.absvalue() <= math.cos(math.pi / 12):
                                    ...
                                    if ptc_gen2.pdgid == 431:
                                            ...
                                            if ptc_gen3.start_vertex == dplus.end_vertex and abs(ptc_gen3.p.pz) / ptc_gen3.p.absvalue() <= math.cos(math.pi / 12):
                                                ...
                                                if abs(ptc_gen3.pdgid) == 15:
                                                    ...
                                                    for ptc_gen4 in ptcs:
                                                        if ptc_gen4.start_vertex == tau_d.end_vertex and abs(ptc_gen4.p.pz) / ptc_gen4.p.absvalue() <= math.cos(math.pi / 12):
                                                            # looking for pi+/-
                                        ...
                                        if len(pis_d) == 3 and pi0_d is not None:
                            ...
                            if b is not None and kstar is not None and k is not None and pi_kstar is not None and dplus is not None and pi1_d is not None and pi2_d is not None and pi3_d is not None and pi0_d is not None and dminus is not None and tau_d is not None and pi1_tau_d is not None and pi2_tau_d is not None and pi3_tau_d is not None and nu_tau_d is not None and nu_d is not None:
