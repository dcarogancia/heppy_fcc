...
class BplusDMuNuXKPiPiAnalyzer(Analyzer):
    def beginLoop(self, setup):
        ...
        self.BdD_counter = 0
        self.Dst_counter = 0
        self.Bd_D0_counter = 0
        self.Bplus_Dmu_counter = 0                   
        self.mu_counter = 0
        self.Bd_mu_counter = 0
        self.Bd_tau_counter = 0
        self.Bplus_pi_counter = 0
        ...
        super(BplusDMuNuXKPiPiAnalyzer, self).beginLoop(setup)
        #self.rootfile = TFile('/'.join([self.dirName, 'Bd2DmunuKpipi-100k.root']), 'recreate')
        
        self.rootfile = TFile('/'.join([self.dirName, 'Bplus_Dmunu_Kpipi_Xpi_evtpdl2019-1M.root']), 'recreate')
    ...
    def process(self, event):
        ...
        Bplus_Kpipimu_spt = 0.0
        Bplus_KpipimuX_spt = 0.0
        Bplus_Kpipimu_mass = 0.0
        Bplus_KpipimuX_mass = 0.0
        Bplus_Kpipimu_smass = 0.0
        Bplus_KpipimuX_smass = 0.0
        
        Kpipimu_par = 0.0
        Kpipimu_pTmis = 0.0
        Bplus_Kpipimu_mcorr = 0.0
        
        KpipimuX_par = 0.0
        KpipimuX_pTmis = 0.0
        Bplus_KpipimuX_mcorr = 0.0
                
        Kpipimu_spar = 0.0
        Kpipimu_spTmis = 0.0
        Bplus_Kpipimu_smcorr = 0.0
        
        KpipimuX_spar = 0.0
        KpipimuX_spTmis = 0.0
        Bplus_KpipimuX_smcorr = 0.0
        ...
        for ptc_gen in ptcs:
            ...
            if abs(ptc_gen.pdgid) == 521:
                ...
                Bplus_fd = np.sqrt((sv_mctruth.x - pv_mctruth.x)**2 + (sv_mctruth.y - pv_mctruth.y)**2 + (sv_mctruth.z - pv_mctruth.z)**2)
                ...
                for ptc_gen1 in ptcs:   
                    ...
                    if (ptc_gen1.start_vertex == sv_mctruth):
                        ...
                        if abs(ptc_gen1.pdgid) == 411:
                            plist_Bplus_D.append(ptc_gen1)   
                            self.Dst_counter += 1
                                                
                        if abs(ptc_gen1.pdgid) == 13:
                            plist_Bplus_mu.append(ptc_gen1)
                            self.Bd_mu_counter += 1
                            
                        if abs(ptc_gen1.pdgid) == 211:
                            plist_Bplus_pi.append(ptc_gen1)
                            self.Bplus_pi_counter += 1
                        
                        if abs(ptc_gen1.pdgid) == 15:
                            plist_Bd_tau.append(ptc_gen1)
                            self.Bd_tau_counter += 1
                        
                        Bplus_pmctruth = ptc_gen1
                        
                        Bplus_X_sp = copy.deepcopy(Bplus_pmctruth)
                                        
                        Bd_pT = np.sqrt(Bplus_pmctruth.p.px**2 + Bplus_pmctruth.p.py**2)
                
                        Bplus_presol = momentum_res(Bd_pT)
                
                        if self.cfg_ana.smear_momentum:
                             Bplus_X_sp.p = smear_momentum(Bplus_X_sp.p, Bplus_presol, Bplus_presol, Bplus_presol)
                        
                        #if ((abs(ptc_gen1.pdgid) not in [11,12,13,14,15,16,411,521]) and (ptc_gen1.status == 1)) :
                        #if (abs(ptc_gen1.pdgid) not in [411,431]) and ptc_gen1.charge != 0:
                        if abs(ptc_gen1.pdgid) == 211:
                             
                            #print 'Particle ID and status', ptc_gen1.pdgid, ptc_gen1.status
                        
                            Bplus_X_Et += Bplus_pmctruth.energy
                            Bplus_X_px += Bplus_pmctruth.p.px
                            Bplus_X_py += Bplus_pmctruth.p.py
                            Bplus_X_pz += Bplus_pmctruth.p.pz
                            Bplus_X_pmag += Bplus_pmctruth.p.absvalue()
                            
                            Bplus_X_sEt += np.sqrt(Bplus_X_sp.p.absvalue()**2 + Bplus_X_sp.mass**2)
                            Bplus_X_spx += Bplus_X_sp.p.px
                            Bplus_X_spy += Bplus_X_sp.p.py
                            Bplus_X_spz += Bplus_X_sp.p.pz
                            Bplus_X_spmag += Bplus_X_sp.p.absvalue()
                            
                        #print ptc_gen1.pdgid                
                
                #if len(plist_Bplus_pi) != 0: 
                #print plist_Bplus_pi[0].pdgid
        
                #if len(plist_Bplus_D) == 1 and len(plist_Bplus_mu) == 1 and len(plist_Bs_D2) == 0:
                if len(plist_Bplus_D) == 1 and len(plist_Bplus_mu) == 1 and len(plist_Bplus_pi) == 1 and self.Bplus_Dmu_counter < 1000000:
                #if len(plist_Bplus_D) == 1 and len(plist_Bplus_mu) == 1 and Bplus_fd > 0.5 and self.Bplus_Dmu_counter < 1000000:
                    
                    self.Bplus_Dmu_counter += 1
        
                    Bplus_D_En = plist_Bplus_D[0].energy
                    Bd_D_px = plist_Bplus_D[0].p.px
                    Bd_D_py = plist_Bplus_D[0].p.py
                    Bd_D_pz = plist_Bplus_D[0].p.pz
                    Bd_D_p = plist_Bplus_D[0].p.absvalue()
        
                    Bd_D = copy.deepcopy(plist_Bplus_D[0])
                    Bd_mu = copy.deepcopy(plist_Bplus_mu[0])
                    Bplus_pi = copy.deepcopy(plist_Bplus_pi[0])                    
                    
        
                    Bd_D_pT = np.sqrt(plist_Bplus_D[0].p.px**2 + plist_Bplus_D[0].p.py**2)
                    Bd_mu_pT = np.sqrt(plist_Bplus_mu[0].p.px**2 + plist_Bplus_mu[0].p.py**2)
                    Bplus_pi_pT = np.sqrt(plist_Bplus_pi[0].p.px**2 + plist_Bplus_pi[0].p.py**2)
        
                    Bplus_D_presol = momentum_res(Bd_D_pT)
                    Bplus_mu_presol = momentum_res(Bd_mu_pT)
                    Bplus_pi_presol = momentum_res(Bplus_pi_pT)
        
                    if self.cfg_ana.smear_momentum:
                        Bd_D.p = smear_momentum(Bd_D.p, Bplus_D_presol, Bplus_D_presol, Bplus_D_presol)
                        Bd_mu.p = smear_momentum(Bd_mu.p, Bplus_mu_presol, Bplus_mu_presol, Bplus_mu_presol) 
                        Bplus_pi.p = smear_momentum(Bplus_pi.p, Bplus_pi_presol, Bplus_pi_presol, Bplus_pi_presol) 
                        
                    #Bd_pt = np.dot([plist_Bplus_D[0].p.px + plist_Bplus_mu[0].p.px + Bplus_X_px, plist_Bplus_D[0].p.py + plist_Bplus_mu[0].p.py + Bplus_X_py, plist_Bplus_D[0].p.pz + plist_Bplus_mu[0].p.pz + Bplus_X_pz],[plist_Bplus_D[0].p.px + plist_Bplus_mu[0].p.px + Bplus_X_px, plist_Bplus_D[0].p.py + plist_Bplus_mu[0].p.py + Bplus_X_py, plist_Bplus_D[0].p.pz + plist_Bplus_mu[0].p.pz + Bplus_X_pz])
                    
                    Bd_pt = np.dot([plist_Bplus_D[0].p.px + plist_Bplus_mu[0].p.px + plist_Bplus_pi[0].p.px, plist_Bplus_D[0].p.py + plist_Bplus_mu[0].p.py + plist_Bplus_pi[0].p.py, plist_Bplus_D[0].p.pz + plist_Bplus_mu[0].p.pz + plist_Bplus_pi[0].p.pz],[plist_Bplus_D[0].p.px + plist_Bplus_mu[0].p.px + plist_Bplus_pi[0].p.px, plist_Bplus_D[0].p.py + plist_Bplus_mu[0].p.py + plist_Bplus_pi[0].p.py, plist_Bplus_D[0].p.pz + plist_Bplus_mu[0].p.pz + plist_Bplus_pi[0].p.pz])
                    
                    
                    Bplus_mass = np.sqrt((Bplus_D_En + plist_Bplus_mu[0].energy + plist_Bplus_pi[0].energy)**2 - Bd_pt)                                     
                    
                    Bd_spt = np.dot([Bd_D.p.px + Bd_mu.p.px + Bplus_pi.p.px, Bd_D.p.py + Bd_mu.p.py + Bplus_pi.p.py, Bd_D.p.pz + Bd_mu.p.pz + Bplus_pi.p.pz],[Bd_D.p.px + Bd_mu.p.px + Bplus_pi.p.px, Bd_D.p.py + Bd_mu.p.py + Bplus_pi.p.py, Bd_D.p.pz + Bd_mu.p.pz + Bplus_pi.p.pz])
                    
                    Bplus_D_sEn = np.sqrt(Bd_D.p.absvalue()**2 + Bd_D.mass**2)
                    
                    Bplus_mu_sEn = np.sqrt(Bd_mu.p.absvalue()**2 + Bd_mu.mass**2)
                    
                    Bplus_pi_sEn = np.sqrt(Bplus_pi.p.absvalue()**2 + Bplus_pi.mass**2)
                
                    Bplus_smass = np.sqrt((Bplus_D_sEn + Bplus_mu_sEn + Bplus_pi_sEn)**2 - Bd_spt)
                    
                    #print plist_Bplus_D[0].p.absvalue(), Bd_D.p.absvalue(), np.sqrt(np.dot([Bd_D.p.px, Bd_D.p.py, Bd_D.p.pz],[Bd_D.p.px, Bd_D.p.py, Bd_D.p.pz]))
                                
                    #if Bplus_smass > 5.5:
                    #    print 'Mass', Bplus_mass, Bplus_smass, Bplus_X_sEt, plist_Bd[0].p.absvalue()
        
                    for ptc_gen2 in ptcs:
                    
                        if (len(plist_Bplus_D) == 1 and (ptc_gen2.start_vertex == plist_Bplus_D[0].end_vertex)):
                            
                            plist_D_Kpipi.append(ptc_gen2)
                            
                            #if abs(ptc_gen2.pdgid) == 211:
                            #print 'Particle ID: ', ptc_gen2.pdgid
        
                        ##comment this
                        
                        #if (len(plist_Bplus_mu) == 1 and (ptc_gen2.start_vertex == plist_Bplus_mu[0].end_vertex) and (ptc_gen2.start_vertex != #ptc_gen2.end_vertex)):
                        #if (len(plist_Bplus_mu) == 1 and (ptc_gen2.start_vertex == plist_Bplus_mu[0].end_vertex)):
                        #    if abs(ptc_gen2.pdgid) == 13:
                        #        plist_Bplus_mu.append(ptc_gen2)
                        #        
                        #        self.mu_counter += 1
                        #    
                    ...
                    #print 'D daughters ' , plist_D_Kpipi[0].pdgid, plist_D_Kpipi[1].pdgid
                    #print 'D daughters status ' , plist_D_Kpipi[0].status, plist_D_Kpipi[1].status, plist_D_Kpipi[2].status
                    #print 'D daughters status ' , plist_D_Kpipi[0].pdgid, plist_D_Kpipi[1].pdgid, plist_D_Kpipi[2].pdgid
                    ...
                    D_mu_px, D_mu_py, D_mu_pz = plist_Bplus_mu[0].p.px, plist_Bplus_mu[0].p.py, plist_Bplus_mu[0].p.pz
                    Bplus_pi_px, Bplus_pi_py, Bplus_pi_pz = plist_Bplus_pi[0].p.px, plist_Bplus_pi[0].p.py, plist_Bplus_pi[0].p.pz
                    
                    D_K_p, D_pi1_p, D_pi2_p, D_mu_p = plist_D_Kpipi[0].p.absvalue(), plist_D_Kpipi[1].p.absvalue(), plist_D_Kpipi[2].p.absvalue(), plist_Bplus_mu[0].p.absvalue()
                    
                    D_K_En, D_pi1_En, D_pi2_En, D_mu_En, Bplus_pi_En = plist_D_Kpipi[0].energy, plist_D_Kpipi[1].energy, plist_D_Kpipi[2].energy, plist_Bplus_mu[0].energy, plist_Bplus_pi[0].energy
                                        
                    D_Kpipimu_En = D_K_En + D_pi1_En + D_pi2_En + D_mu_En
        
                    #Bplus_Kpipimu_pt = np.dot([plist_D_Kpipi[0].p.px + plist_D_Kpipi[1].p.px + plist_D_Kpipi[2].p.px + plist_Bplus_mu[0].p.px + Bplus_X_px, plist_D_Kpipi[0].p.py + plist_D_Kpipi[1].p.py + plist_D_Kpipi[2].p.py + plist_Bplus_mu[0].p.py + Bplus_X_py, plist_D_Kpipi[0].p.pz + plist_D_Kpipi[1].p.pz + plist_D_Kpipi[2].p.pz + plist_Bplus_mu[0].p.pz + Bplus_X_pz],[plist_D_Kpipi[0].p.px + plist_D_Kpipi[1].p.px + plist_D_Kpipi[2].p.px + plist_Bplus_mu[0].p.px + Bplus_X_px, plist_D_Kpipi[0].p.py + plist_D_Kpipi[1].p.py + plist_D_Kpipi[2].p.py + plist_Bplus_mu[0].p.py + Bplus_X_py, plist_D_Kpipi[0].p.pz + plist_D_Kpipi[1].p.pz + plist_D_Kpipi[2].p.pz + plist_Bplus_mu[0].p.pz + Bplus_X_pz])
                    
                    Bplus_Kpipimu_pt = np.dot([D_K_px + D_pi1_px + D_pi2_px + D_mu_px, D_K_py + D_pi1_py + D_pi2_py + D_mu_py, D_K_pz + D_pi1_pz + D_pi2_pz + D_mu_pz],[D_K_px + D_pi1_px + D_pi2_px + D_mu_px, D_K_py + D_pi1_py + D_pi2_py + D_mu_py, D_K_pz + D_pi1_pz + D_pi2_pz + D_mu_pz])
                                                            
                    Bplus_KKpimuX_pt = np.dot([D_K_px + D_pi1_px + D_pi2_px + D_mu_px + Bplus_pi_px, D_K_py + D_pi1_py + D_pi2_py + D_mu_py + Bplus_pi_py , D_K_pz + D_pi1_pz + D_pi2_pz + D_mu_pz + Bplus_pi_pz],[D_K_px + D_pi1_px + D_pi2_px + D_mu_px + Bplus_pi_px, D_K_py + D_pi1_py + D_pi2_py + D_mu_py + Bplus_pi_py , D_K_pz + D_pi1_pz + D_pi2_pz + D_mu_pz + Bplus_pi_pz])
                    
                    Bplus_Kpipimu_mass = np.sqrt((D_Kpipimu_En)**2 - Bplus_Kpipimu_pt)
                    
                    Bplus_KpipimuX_mass = np.sqrt((D_Kpipimu_En + Bplus_pi_En)**2 - Bplus_KKpimuX_pt)
        
                    D_K = copy.deepcopy(plist_D_Kpipi[0])
                    D_pi1 = copy.deepcopy(plist_D_Kpipi[1])
                    D_pi2 = copy.deepcopy(plist_D_Kpipi[2])
                    D_mu  = copy.deepcopy(plist_Bplus_mu[0])
                    ...
                    D_mu_pT  = np.sqrt(plist_Bplus_mu[0].p.px**2 + plist_Bplus_mu[0].p.py**2)
                    ...
                    Bplus_pi_spx, Bplus_pi_spy, Bplus_pi_spz = Bplus_pi.p.px, Bplus_pi.p.py, Bplus_pi.p.pz
                    ...
                    Bplus_Kpipimu_spt = np.dot([D_K_spx + D_pi1_spx + D_pi2_spx + D_mu_spx, D_K_spy + D_pi1_spy + D_pi2_spy + D_mu_spy, D_K_spz + D_pi1_spz + D_pi2_spz + D_mu_spz],[D_K_spx + D_pi1_spx + D_pi2_spx + D_mu_spx, D_K_spy + D_pi1_spy + D_pi2_spy + D_mu_spy, D_K_spz + D_pi1_spz + D_pi2_spz + D_mu_spz])
                    
                    Bplus_KpipimuX_spt = np.dot([D_K_spx + D_pi1_spx + D_pi2_spx + D_mu_spx + Bplus_pi_spx, D_K_spy + D_pi1_spy + D_pi2_spy + D_mu_spy + Bplus_pi_spy , D_K_spz + D_pi1_spz + D_pi2_spz + D_mu_spz + Bplus_pi_spz],[D_K_spx + D_pi1_spx + D_pi2_spx + D_mu_spx + Bplus_pi_spx, D_K_spy + D_pi1_spy + D_pi2_spy + D_mu_spy + Bplus_pi_spy , D_K_spz + D_pi1_spz + D_pi2_spz + D_mu_spz + Bplus_pi_spz])
                    
                    #Bplus_KpipimuX_spt = np.dot([D_K_spx + D_pi1_spx + D_pi2_spx + D_mu_spx + Bplus_X_spx + D_X_spx, D_K_spy + D_pi1_spy + D_pi2_spy + D_mu_spy + Bplus_X_spy + D_X_spy, D_K_spz + D_pi1_spz + D_pi2_spz + D_mu_spz + Bplus_X_spz + D_X_spz],[D_K_spx + D_pi1_spx + D_pi2_spx + D_mu_spx + Bplus_X_spx + D_X_spx, D_K_spy + D_pi1_spy + D_pi2_spy + D_mu_spy + Bplus_X_spy + D_X_spy, D_K_spz + D_pi1_spz + D_pi2_spz + D_mu_spz + Bplus_X_spz + D_X_spz])
                    
                    #Bplus_KpipimuX_spt = np.dot([D_K.p.px + D_pi1.p.px + D_pi2.p.px + D_mu.p.px, D_K.p.py + D_pi1.p.py + D_pi2.p.py + D_mu.p.py, D_K.p.pz + D_pi1.p.pz + D_pi2.p.pz + D_mu.p.pz],[D_K.p.px + D_pi1.p.px + D_pi2.p.px + D_mu.p.px, D_K.p.py + D_pi1.p.py + D_pi2.p.py + D_mu.p.py, D_K.p.pz + D_pi1.p.pz + D_pi2.p.pz + D_mu.p.pz])

                    #Bplus_Kpipimu_smass = np.sqrt((D_Kpipimu_sEn + Bplus_X_sEt)**2 - Bplus_Kpipimu_spt)
                    Bplus_Kpipimu_smass = np.sqrt((D_Kpipimu_sEn)**2 - Bplus_Kpipimu_spt)
                    
                    Bplus_KpipimuX_smass = np.sqrt((D_Kpipimu_sEn + Bplus_pi_sEn)**2 - Bplus_KpipimuX_spt)
                    ...
                    Kpipimu_par = np.dot([sv_mctruth.x - pv_mctruth.x, sv_mctruth.y - pv_mctruth.y, sv_mctruth.z - pv_mctruth.z],[D_K_px + D_pi1_px + D_pi2_px + D_mu_px, D_K_py + D_pi1_py + D_pi2_py + D_mu_py, D_K_pz + D_pi1_pz + D_pi2_pz + D_mu_pz])/Bplus_fd

                    Kpipimu_pTmis = np.dot([(D_K_px + D_pi1_px + D_pi2_px + D_mu_px) - Kpipimu_par*(sv_mctruth.x - pv_mctruth.x)/Bplus_fd, (D_K_py + D_pi1_py + D_pi2_py + D_mu_py) - Kpipimu_par*(sv_mctruth.y - pv_mctruth.y)/Bplus_fd, (D_K_pz + D_pi1_pz + D_pi2_pz + D_mu_pz) - Kpipimu_par*(sv_mctruth.z - pv_mctruth.z)/Bplus_fd],[(D_K_px + D_pi1_px + D_pi2_px + D_mu_px) - Kpipimu_par*(sv_mctruth.x - pv_mctruth.x)/Bplus_fd, (D_K_py + D_pi1_py + D_pi2_py + D_mu_py) - Kpipimu_par*(sv_mctruth.y - pv_mctruth.y)/Bplus_fd, (D_K_pz + D_pi1_pz + D_pi2_pz + D_mu_pz) - Kpipimu_par*(sv_mctruth.z - pv_mctruth.z)/Bplus_fd])
                            
                    Bplus_Kpipimu_mcorr = np.sqrt(Bplus_Kpipimu_mass**2 + Kpipimu_pTmis) + np.sqrt(Kpipimu_pTmis)
                    
                    KpipimuX_par = np.dot([sv_mctruth.x - pv_mctruth.x, sv_mctruth.y - pv_mctruth.y, sv_mctruth.z - pv_mctruth.z],[D_K_px + D_pi1_px + D_pi2_px + D_mu_px + Bplus_pi_px, D_K_py + D_pi1_py + D_pi2_py + D_mu_py + Bplus_pi_py, D_K_pz + D_pi1_pz + D_pi2_pz + D_mu_pz + Bplus_pi_pz])/Bplus_fd

                    KpipimuX_pTmis = np.dot([(D_K_px + D_pi1_px + D_pi2_px + D_mu_px + Bplus_pi_px) - KpipimuX_par*(sv_mctruth.x - pv_mctruth.x)/Bplus_fd, (D_K_py + D_pi1_py + D_pi2_py + D_mu_py + Bplus_pi_py) - KpipimuX_par*(sv_mctruth.y - pv_mctruth.y)/Bplus_fd, (D_K_pz + D_pi1_pz + D_pi2_pz + D_mu_pz + Bplus_pi_pz) - KpipimuX_par*(sv_mctruth.z - pv_mctruth.z)/Bplus_fd],[(D_K_px + D_pi1_px + D_pi2_px + D_mu_px + Bplus_pi_px) - KpipimuX_par*(sv_mctruth.x - pv_mctruth.x)/Bplus_fd, (D_K_py + D_pi1_py + D_pi2_py + D_mu_py + Bplus_pi_py) - KpipimuX_par*(sv_mctruth.y - pv_mctruth.y)/Bplus_fd, (D_K_pz + D_pi1_pz + D_pi2_pz + D_mu_pz + Bplus_pi_pz) - KpipimuX_par*(sv_mctruth.z - pv_mctruth.z)/Bplus_fd])
                            
                    Bplus_KpipimuX_mcorr = np.sqrt(Bplus_KpipimuX_mass**2 + KpipimuX_pTmis) + np.sqrt(KpipimuX_pTmis)
                    ...
                    Bplus_Kpipimu_smcorr = np.sqrt(Bplus_Kpipimu_smass**2 + Kpipimu_spTmis) + np.sqrt(Kpipimu_spTmis)
                    
                    KpipimuX_spar = np.dot([sv.x - pv.x, sv.y - pv.y, sv.z - pv.z],[D_K_spx + D_pi1_spx + D_pi2_spx + D_mu_spx + Bplus_pi_spx, D_K_spy + D_pi1_spy + D_pi2_spy + D_mu_spy + Bplus_pi_spy, D_K_spz + D_pi1_spz + D_pi2_spz + D_mu_spz + Bplus_pi_spz])/Bd_sfd

                    KpipimuX_spTmis = np.dot([(D_K_spx + D_pi1_spx + D_pi2_spx + D_mu_spx + Bplus_pi_spx) - KpipimuX_spar*(sv.x - pv.x)/Bd_sfd, (D_K_spy + D_pi1_spy + D_pi2_spy + D_mu_spy + Bplus_pi_spy) - KpipimuX_spar*(sv.y - pv.y)/Bd_sfd, (D_K_spz + D_pi1_spz + D_pi2_spz + D_mu_spz + Bplus_pi_spz) - KpipimuX_spar*(sv.z - pv.z)/Bd_sfd],[(D_K_spx + D_pi1_spx + D_pi2_spx + D_mu_spx + Bplus_pi_spx) - KpipimuX_spar*(sv.x - pv.x)/Bd_sfd, (D_K_spy + D_pi1_spy + D_pi2_spy + D_mu_spy + Bplus_pi_spy) - KpipimuX_spar*(sv.y - pv.y)/Bd_sfd, (D_K_spz + D_pi1_spz + D_pi2_spz + D_mu_spz + Bplus_pi_spz) - KpipimuX_spar*(sv.z - pv.z)/Bd_sfd])
                            
                    Bplus_KpipimuX_smcorr = np.sqrt(Bplus_KpipimuX_smass**2 + KpipimuX_spTmis) + np.sqrt(KpipimuX_spTmis)
                    ...
                    self.mc_truth_tree.fill('sv_Ds_y',          sv_Ds_mctruth.y)
                    self.mc_truth_tree.fill('sv_Ds_z',          sv_Ds_mctruth.z)
                    self.mc_truth_tree.fill('pvsv_distance',    Bplus_fd)
                    ...
                    self.mc_truth_tree.fill('B_Ds_En',          Bplus_D_En)
                    ...
                    self.mc_truth_tree.fill('B_KKpimu_mass',   Bplus_Kpipimu_mass)
                    self.mc_truth_tree.fill('B_Kpipimu_mcorr',  Bplus_Kpipimu_mcorr)
                    ...
                    self.smeared_tree.fill('B_Ds_sEn',           Bplus_D_sEn)
                    ...
                    self.smeared_tree.fill('KpipimuX_spar',       KpipimuX_spar)
                    self.smeared_tree.fill('KpipimuX_spTmis',     np.sqrt(KpipimuX_spTmis))
                    self.smeared_tree.fill('B_Kpipimu_mass',      Bplus_Kpipimu_mass)
                    self.smeared_tree.fill('B_Kpipimu_smass',     Bplus_Kpipimu_smass)
                    self.smeared_tree.fill('B_Kpipimu_mcorr',     Bplus_Kpipimu_mcorr)
                    self.smeared_tree.fill('B_Kpipimu_smcorr',    Bplus_Kpipimu_smcorr)
                    self.smeared_tree.fill('B_KpipimuX_mass',     Bplus_KpipimuX_mass)
                    self.smeared_tree.fill('B_KpipimuX_smass',    Bplus_KpipimuX_smass)
                    self.smeared_tree.fill('B_KpipimuX_mcorr',    Bplus_KpipimuX_mcorr)
                    self.smeared_tree.fill('B_KpipimuX_smcorr',   Bplus_KpipimuX_smcorr)
    ...
    def write(self, unusefulVar):
        ...
        print('Bplus events with momentum cut: {}'.format(self.pb_counter))
        print('Dmupi from Bplus: {}'.format(self.Bplus_Dmu_counter))
        print('D(+/-) from Bplus: {}'.format(self.Dst_counter))
        print('Muons from Bplus meson: {}'.format(self.Bd_mu_counter))
        print('Tau from Bplus meson: {}'.format(self.Bd_tau_counter))              
