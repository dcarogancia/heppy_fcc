                ...
                if abs(ptc_gen1.p.pz) / ptc_gen1.p.absvalue() <= math.cos(math.pi / 12):
                    ...
                    if pb > 25.: # select only events with large momentum of the B
                        ...
                        if pvsv_distance > 1.: # select only events with long flight distance of the B
                                ...
                                if ptc_gen2.start_vertex == b.end_vertex and abs(ptc_gen2.p.pz) / ptc_gen2.p.absvalue() <= math.cos(math.pi / 12):
                                    ...
                                    if ptc_gen2.pdgid == -15:
                                        ...
                                        for ptc_gen3 in ptcs:
                                            if  ptc_gen3.start_vertex == tauplus.end_vertex and abs(ptc_gen3.p.pz) / ptc_gen3.p.absvalue() <= math.cos(math.pi / 12):
                                    ...
                                    if ptc_gen2.pdgid == 15:
                                        ...
                                        pis_tauminus = []
                                        for ptc_gen3 in ptcs:
                                            if  ptc_gen3.start_vertex == tauminus.end_vertex and abs(ptc_gen3.p.pz) / ptc_gen1.p.absvalue() <= math.cos(math.pi / 12):
                                                # looking for pions from tau+ decay
                                                if abs(ptc_gen3.pdgid) == 211:
                            ...
                            if b is not None and kstar is not None and tauplus is not None and tauminus is not None and pi1_tauplus is not None and pi2_tauplus is not None and pi3_tauplus is not None and nu_tauplus is not None and pi1_tauminus is not None and pi2_tauminus is not None and pi3_tauminus is not None and nu_tauminus is not None and k is not None and pi_kstar is not None:
