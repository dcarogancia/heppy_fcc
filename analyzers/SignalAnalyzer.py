...
class SignalAnalyzer(Analyzer):
	"""
		...
		Inherits from heppy.framework.analyzer.Analyzer. Overrides base class methods to cover our needs
	"""
	...
	def beginLoop(self, setup):
		...
		TH1F.AddDirectory(False) # not to link histogram to files
				...
				if pb > 25.: # select only events with large momentum of the B
					...
					if pvsv_distance > 1.: # select only events with long flight distance of the B
						...
						for ptc_gen2 in ptcs:
							if ptc_gen2.start_vertex == b.end_vertex:
								...
								if ptc_gen2.pdgid == -15:
									...
									svtv_tauplus_distance = math.sqrt((tv_tauplus.x - sv.x) ** 2 + (tv_tauplus.y - sv.y) ** 2 + (tv_tauplus.z - sv.z) ** 2)

								# looking for tau-
								if ptc_gen2.pdgid == 15:
									tauminus = ptc_gen2
									tv_tauminus = tauminus.end_vertex
									svtv_tauminus_distance = math.sqrt((tv_tauminus.x - sv.x) ** 2 + (tv_tauminus.y - sv.y) ** 2 + (tv_tauminus.z - sv.z) ** 2)
						...
						if max(svtv_tauplus_distance, svtv_tauminus_distance) > 0.5: # select only events with long flight distance of tau
							...
							self.tree.fill('sv_x', numpy.random.normal(sv.x, self.cfg_ana.pv_x_resolution) if self.cfg_ana.smear_sv else sv.x)
							self.tree.fill('sv_y', numpy.random.normal(sv.y, self.cfg_ana.pv_y_resolution) if self.cfg_ana.smear_sv else sv.y)
							self.tree.fill('sv_z', numpy.random.normal(sv.z, self.cfg_ana.pv_z_resolution) if self.cfg_ana.smear_sv else sv.z)
							self.tree.fill('tv_tauplus_x', numpy.random.normal(tv_tauplus.x, self.cfg_ana.pv_x_resolution) if self.cfg_ana.smear_tv else tv_tauplus.x)
							self.tree.fill('tv_tauplus_y', numpy.random.normal(tv_tauplus.y, self.cfg_ana.pv_y_resolution) if self.cfg_ana.smear_tv else tv_tauplus.y)
							self.tree.fill('tv_tauplus_z', numpy.random.normal(tv_tauplus.z, self.cfg_ana.pv_z_resolution) if self.cfg_ana.smear_tv else tv_tauplus.z)
							self.tree.fill('tv_tauminus_x', numpy.random.normal(tv_tauminus.x, self.cfg_ana.pv_x_resolution) if self.cfg_ana.smear_tv else tv_tauminus.x)
							self.tree.fill('tv_tauminus_y', numpy.random.normal(tv_tauminus.y, self.cfg_ana.pv_y_resolution) if self.cfg_ana.smear_tv else tv_tauminus.y)
							self.tree.fill('tv_tauminus_z', numpy.random.normal(tv_tauminus.z, self.cfg_ana.pv_z_resolution) if self.cfg_ana.smear_tv else tv_tauminus.z)
