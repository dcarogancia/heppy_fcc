...
class BsDsMuNuXKKPiAnalyzer(Analyzer):
    def beginLoop(self, setup):
        ...
        super(BsDsMuNuXKKPiAnalyzer, self).beginLoop(setup)
        
        self.rootfile = TFile('/'.join([self.dirName, 'Bs_DsmunuX_KKpi_signal_evtpdl2019-1M.root']), 'recreate')
        #self.rootfile = TFile('/'.join([self.dirName, 'Bs_DsmunuX_KKpi_signal_evtpdl2019-100k.root']), 'recreate')
        ...
        self.smeared_vtx.var('Dsmu_spTmis')
    ...
    def process(self, event):
        ...
        for ptc_gen0 in ptcs:
            
            if (abs(ptc_gen0.pdgid) == 531 and ptc_gen0.start_vertex != ptc_gen0.end_vertex):
                #plist_B0s.append(ptc_gen0)
                B0s_dvtx = ptc_gen0.end_vertex
                
                for ptc_gen in ptcs:
            
                    if (ptc_gen.start_vertex == B0s_dvtx and ptc_gen.start_vertex != ptc_gen.end_vertex):
                    #if (ptc_gen.start_vertex == B0s_dvtx):
                        plist_B0s.append(ptc_gen)
                        B0s_Et += ptc_gen.energy
                        B0s_px += ptc_gen.p.px
                        B0s_py += ptc_gen.p.py
                        B0s_pz += ptc_gen.p.pz
                ...
                B0s_pt = np.dot([B0s_px , B0s_py, B0s_pz],[B0s_px , B0s_py, B0s_pz])
                
                #if len(plist_B0s) == 3:
                #    B0s_px = plist_B0s[0].p.px + plist_B0s[1].p.px + plist_B0s[2].p.px
                #    B0s_py = plist_B0s[0].p.py + plist_B0s[1].p.py + plist_B0s[2].p.py
                #    B0s_pz = plist_B0s[0].p.pz + plist_B0s[1].p.pz + plist_B0s[2].p.pz
                #    B0s_pt = np.dot([B0s_px , B0s_py, B0s_pz],[B0s_px , B0s_py, B0s_pz])
                
                #if len(plist_B0s) == 3:
                print np.sqrt(B0s_Et**2-B0s_pt)
        #===============================================================================================#
                ...
                if pb > 0.0: # select only events with large momentum of the B
                    ...
                    if pvsv_distance > 0.0:
                        ...
                        if len(plist_Ds_mctruth) == 1 and len(plist_Bmu_mc_truth) == 1:
                            ...
                            mu_sdiffx = sv.x - pv_mu.x
                            mu_sdiffy = sv.y - pv_mu.y
                            mu_sdiffz = sv.z - pv_mu.z
                            
                            mu_sdiffx2 = pv.x - pv_mu.x
                            mu_sdiffy2 = pv.y - pv_mu.y
                            mu_sdiffz2 = pv.z - pv_mu.z
                            ...
                            if len(plist_D2K_mc_truth) == 2 and len(plist_Dpi_mc_truth) == 1:
                                ...
                                self.smeared_vtx.fill('Dsmu_spTmis',        np.sqrt(KKpimu_pTmis))
