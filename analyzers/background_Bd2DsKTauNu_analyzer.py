...
class background_Bd2DsKTauNu_analyzer(Analyzer):
	def beginLoop(self, setup):
		...
		super(background_Bd2DsKTauNu_analyzer, self).beginLoop(setup)
				...
				if pb > 25.: # Select only events with large momentum of the B
					...
					if pvsv_distance > 1.: # Select only events with long flight distance of the B
						...
						for ptc_gen2 in ptcs:
							# looking for tau
							if abs(ptc_gen2.pdgid) == 15 and ptc_gen2.start_vertex == b.end_vertex:
								tau = copy.deepcopy(ptc_gen2) # copy is needed in order to keep initial particle properties after smearing

								if self.cfg_ana.smear_momentum:
									tau.p = smear_momentum(tau.p, self.cfg_ana.momentum_x_resolution, self.cfg_ana.momentum_y_resolution, self.cfg_ana.momentum_z_resolution)

								if tau.pdgid > 0:
									tv_tauminus = copy.deepcopy(tau.end_vertex) # copy is needed in order to keep initial vertex properties after smearing
									...
									svtv_tauminus_distance = math.sqrt((tau.end_vertex.x - b.end_vertex.x) ** 2 + (tau.end_vertex.y - b.end_vertex.y) ** 2 + (tau.end_vertex.z - b.end_vertex.z) ** 2)

									# looking for pis_tauminus
									pis_tauminus = list([])
									for ptc_gen3 in ptcs:
										if abs(ptc_gen3.pdgid) == 211 and ptc_gen3.start_vertex == tau.end_vertex:
											pi_tauminus = copy.deepcopy(ptc_gen3) # copy is needed in order to keep initial particle properties after smearing
									...
									tv_tauplus = copy.deepcopy(tau.end_vertex) # copy is needed in order to keep initial vertex properties after smearing
									...
									svtv_tauplus_distance = math.sqrt((tau.end_vertex.x - b.end_vertex.x) ** 2 + (tau.end_vertex.y - b.end_vertex.y) ** 2 + (tau.end_vertex.z - b.end_vertex.z) ** 2) # Use TV_tauPlus/SV instead of tauPlus.end_vertex/tauPlus.start_vertex

									# looking for pis_tauplus
									pis_tauplus = list([])
									for ptc_gen3 in ptcs:
										if abs(ptc_gen3.pdgid) == 211 and ptc_gen3.start_vertex == tau.end_vertex:
											pi_tauplus = copy.deepcopy(ptc_gen3) # copy is needed in order to keep initial particle properties after smearing
							...
							# looking for D
							if abs(ptc_gen2.pdgid) == 431 and ptc_gen2.start_vertex == b.end_vertex:
								# looking for tau
								for ptc_gen3 in ptcs:
									if abs(ptc_gen3.pdgid) == 15 and ptc_gen3.start_vertex == ptc_gen2.end_vertex:
										tau = copy.deepcopy(ptc_gen3) # copy is needed in order to keep initial particle properties after smearing
										...
										if tau.pdgid > 0:
											...
											for ptc_gen4 in ptcs:
												if abs(ptc_gen4.pdgid) == 211 and ptc_gen4.start_vertex == tau.end_vertex:
											...
											tv_tauplus = copy.deepcopy(tau.end_vertex)

											if self.cfg_ana.smear_tv:
												tv_tauplus = smear_vertex(tv_tauplus, self.cfg_ana.tv_x_resolution, self.cfg_ana.tv_y_resolution, self.cfg_ana.tv_z_resolution)

											svtv_tauplus_distance = math.sqrt((tau.end_vertex.x - b.end_vertex.x) ** 2 + (tau.end_vertex.y - b.end_vertex.y) ** 2 + (tau.end_vertex.z - b.end_vertex.z) ** 2)
						...
						print b.is_valid(), k.is_valid(), pi_k.is_valid(), tauplus.is_valid(), tauminus.is_valid(), pi1_tauplus.is_valid(), pi2_tauplus.is_valid(), pi3_tauplus.is_valid(), pi1_tauminus.is_valid(), pi2_tauminus.is_valid(), pi3_tauminus.is_valid()
