                ...
                if abs(ptc_gen1.p.pz) / ptc_gen1.p.absvalue() <= math.cos(math.pi / 12):
                    ...
                    if pb > 25.: # Select only events with large momentum of the B
                        ...
                        if pvsv_distance > 1.: # Select only events with long flight distance of the B
                                                ...
                                                if ptc_gen3.pdgid == -15:
                                                    ...
                                                    for ptc_gen4 in ptcs:
                                                        if ptc_gen4.start_vertex == tauplus.end_vertex:
                                                            # looking for pi+/-
                                                            if abs(ptc_gen4.pdgid) == 211 and abs(ptc_gen4.p.pz) / ptc_gen4.p.absvalue() <= math.cos(math.pi / 12):
                                                                pis_tauplus.append(ptc_gen4)

                                                            # looking for nu from tau+ decay
                                                            if ptc_gen4.pdgid == -16 and abs(ptc_gen4.p.pz) / ptc_gen4.p.absvalue() <= math.cos(math.pi / 12):
                                                                nu_tauplus = ptc_gen4
                            ...
                            if b is not None and kstar is not None and k is not None and pi_kstar is not None and dplus is not None and dminus is not None and tauplus is not None and tauminus is not None and nu_dplus is not None and nu_dminus is not None and pi1_tauplus is not None and pi2_tauplus is not None and pi3_tauplus is not None and nu_tauplus is not None and pi1_tauminus is not None and pi2_tauminus is not None and pi3_tauminus is not None and nu_tauminus is not None:
